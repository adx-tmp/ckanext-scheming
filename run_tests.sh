...
# Output is both displayed and saved to results.txt
act pull_request -j test --pull=false 2>&1 | tee results.txt
...
echo "Full output saved to: results.txt"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "❌ Tests failed. Check results.txt for details."
fi
